...
async def start_situational_session() -> CreateSessionResponse:
    ...
    try:
        response = await provider.generate(
            [Message(role=Role.SYSTEM, content=_INTERVIEWER_SYSTEM_PROMPT)],
            temperature=0.9,
    ...
    session = create_session(question=question)
